...
+ (Solar *)lunarToSolar:(Lunar *)lunar {
    ...
    //判断是否是闰月
    lunar.isLeap = NO;
    NSString *yearKey = [NSString stringWithFormat:@"%i", (int)lunar.lunarYear];
    ...
    NSLog(@"阴历: %i年 %i月 %i日 是否闰月:%i", (int)lunar.lunarYear, (int)lunar.lunarMonth, (int)lunar.lunarDay, (int)lunar.isLeap);
    if (!lunar.isLeap) {
...
+ (Lunar *)solarToLunar:(Solar *)solar {
    ...
    int m = GetBitInt(solar11, 4, 5);
    int d = GetBitInt(solar11, 5, 0);
    long offset = SolarToInt((int)solar.solarYear, (int)solar.solarMonth, (int)solar.solarDay) - SolarToInt(y, m, d);
